...
`dat_linter.toml` を自動探索します（存在しなければ全ルール有効・`language=en`のデフォルト設定の
まま動作します。**自動生成はしません**）。

コメント付きのデフォルト設定ファイルをカレントディレクトリに生成したい場合は、明示的に
`dat_linter init` を実行してください。

```
dat_linter init
```

既に `dat_linter.toml` が存在する場合は上書きせず、失敗終了します。
