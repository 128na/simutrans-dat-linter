...
#[test]
fn valid_smoke_has_no_errors_or_warnings() {
    let diags = check("smoke_valid.dat");
...
fn dash_sentinel_is_not_an_error() {
    let diags = check("smoke_dash_sentinel_valid.dat");
    let errors: Vec<_> = diags
        .iter()
        .filter(|(s, _)| *s == Severity::Error)
        .collect();
...
fn no_images_is_not_an_error() {
    let diags = check("smoke_no_images.dat");
...
fn missing_image_file_is_detected() {
    assert!(has_error(
        &check("smoke_missing_image_file.dat"),
        "missing-image-file"
    ));
}
...
fn bad_image_size_is_detected() {
    assert!(has_error(
        &check("smoke_bad_image_size.dat"),
...
/// menuマイルストーンで既に実装済みのロジックがsmokeにもそのまま適用される）。
#[test]
fn zoomable_prefix_is_not_a_false_positive() {
    let diags = check("smoke_zoomable_prefix_valid.dat");
    let errors: Vec<_> = diags
        .iter()
        .filter(|(s, _)| *s == Severity::Error)
        .collect();
