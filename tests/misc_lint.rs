//! `rules::check_misc` の統合テスト。testdata/ の正常系3件（通常画像/`"-"`
//! センチネル混在/`"> "`ズーム不可プレフィックス）・異常系2件で、期待する診断コードが
//! 出る（または全く出ない）ことを確認する。`tests/menu_lint.rs`/`tests/cursor_lint.rs`/
//! `tests/symbol_lint.rs`/`tests/smoke_lint.rs`/`tests/field_lint.rs`と同じ形式に
//! 揃えている（`miscimages_writer_t`は`menuskin_writer_t`/`cursorskin_writer_t`/
//! `symbolskin_writer_t`/`smoke_writer_t`/`field_writer_t`と挙動上完全に同一のため）。
...
fn dash_sentinel_is_not_an_error() {
    let diags = check("misc_dash_sentinel_valid.dat");
    let errors: Vec<_> = diags
        .iter()
        .filter(|(s, _)| *s == Severity::Error)
        .collect();
...
fn no_images_is_not_an_error() {
    let diags = check("misc_no_images.dat");
...
fn missing_image_file_is_detected() {
    assert!(has_error(
        &check("misc_missing_image_file.dat"),
        "missing-image-file"
    ));
}
...
/// 誤検知してはならない（`common::check_image_ref`の`strip_zoomable_prefix_and_trim`で
/// 対応、menuマイルストーンで既に実装済みのロジックがmiscにもそのまま適用される）。
#[test]
fn zoomable_prefix_is_not_a_false_positive() {
    let diags = check("misc_zoomable_prefix_valid.dat");
    let errors: Vec<_> = diags
        .iter()
        .filter(|(s, _)| *s == Severity::Error)
        .collect();
