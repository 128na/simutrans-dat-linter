...
const FORMAT_HELP_JA: &str = "出力形式。textはこれまで通りのeprintln/println出力、jsonは診断・サマリをまとめて1回だけstdoutへ構造化出力する（stderrへは何も出さない）";
const FORMAT_HELP_EN: &str = "Output format. text is the existing eprintln/println output; json emits diagnostics and the summary as a single structured payload to stdout only (nothing is written to stderr)";
    ...
    cmd.about(top_about)
        .mut_subcommand("lint", |c| {
            ...
            let format_help = match lang {
                Language::Japanese => FORMAT_HELP_JA,
                Language::English => FORMAT_HELP_EN,
            };
            c.about(about)
                ...
                .mut_arg("format", |a| a.help(format_help))
        })
...
pub struct LintArgs {
    ...
    /// 出力形式。省略時は`text`（従来通りのeprintln/println出力）。
    /// `json`はVSCode拡張等の機械可読な消費者向けに、診断・サマリをまとめて
    /// stdoutへ1回だけ構造化出力する（第1弾: VSCode拡張のDiagnostics統合）。
    #[arg(long, value_enum, default_value_t = LintFormat::Text)]
    pub format: LintFormat,
}

/// `lint --format`が選べる出力形式。
#[derive(Debug, Clone, Copy, PartialEq, Eq, clap::ValueEnum, Default)]
pub enum LintFormat {
    /// 従来通りの`eprintln!`（診断ごと）/`println!`（サマリ）出力。デフォルト。
    #[default]
    Text,
    /// 診断・サマリをまとめて1回の`serde_json::to_string`でstdoutへ出力する
    /// （stderrへは何も出さない）。
    Json,
}
