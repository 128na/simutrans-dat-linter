// building/vehicle/way/good/bridge/tunnel の検証ロジック（cursor/icon省略時のスキップ、タイル画像欠落時の
...
// 季節数/portal幅の可変画像キー走査等）は、いずれもmakeobjの
// building_writer.cc / vehicle_writer.cc / way_writer.cc / good_writer.cc /
// bridge_writer.cc / tunnel_writer.cc / get_waytype.cc / xref_writer.cc / tabfile.cc を
// ソースとして直接ミラーしている。
//
// 検証済み:
...
// タイル画像・vehicle画像・way画像・goodフィールド・tunnel画像キーのロジックが
// 変わった場合はこの定数表を再検証すること。vehicle系・way系・good系・bridge系・
// tunnel系のルールはOTRP側での個別diffはまだ行っていない
// （rules/vehicle.rs, rules/way.rs, rules/good.rs, rules/bridge.rs, rules/tunnel.rs参照）。
