// building/vehicle/way/good/bridge/tunnel/roadsign の検証ロジック（cursor/icon省略時のスキップ、タイル画像欠落時の
...
// fatal分岐再現等）は、いずれもmakeobjの
// building_writer.cc / vehicle_writer.cc / way_writer.cc / good_writer.cc /
// bridge_writer.cc / tunnel_writer.cc / roadsign_writer.cc / get_waytype.cc /
...
// タイル画像・vehicle画像・way画像・goodフィールド・tunnel画像キー・roadsign画像キーの
// ロジックが変わった場合はこの定数表を再検証すること。vehicle系・way系・good系・bridge系・
// tunnel系・roadsign系のルールはOTRP側での個別diffはまだ行っていない
// （rules/vehicle.rs, rules/way.rs, rules/good.rs, rules/bridge.rs, rules/tunnel.rs,
// rules/roadsign.rs参照）。
