// building/vehicle/way/good/bridge/tunnel/roadsign/crossing/way-object/ground_obj/tree/citycar/pedestrian/factory の検証ロジック（cursor/icon省略時のスキップ、タイル画像欠落時の
...
// error・probability系クランプ等）は、いずれもmakeobjの
...
// pedestrian_writer.cc / factory_writer.cc / get_waytype.cc / xref_writer.cc /
// tabfile.cc を
// ソースとして直接ミラーしている。
//
// 検証済み:
...
// pedestrian画像キー・factoryフィールドのロジックが変わった場合はこの定数表を再検証すること。
// vehicle系・way系・good系・bridge系・tunnel系・roadsign系・crossing系・way-object系・
// ground_obj系・tree系・citycar系・pedestrian系・factory系のルールはOTRP側での個別diffは
