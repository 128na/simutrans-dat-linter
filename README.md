...
`lint`/`fmt`/`couplings` のいずれかを実行すると、コメント付きのデフォルト設定ファイルを
カレントディレクトリに自動生成します（`--help` 表示時も含め、起動のたびに探索されるタイミングで
生成されます）。生成に失敗しても（書き込み権限が無い等）致命的エラーにはせず、
設定ファイル無しの状態のまま動作を継続します。生成される内容は次の「出力言語」節の
`language = "en"` と `[rules] include/exclude` の空配列のみです。
...
`lint`/`fmt`/`couplings` の全サブコマンドが `--config`（またはカレントディレクトリの
...
- `power` を宣言しているのに `gear` が変換後 `(gear*64/100)` で 0 になる（`gear=0` または `gear=1`）
  ...
  （`simconvoi.cc`）を根拠とする「静的解析」層のルールです（`couplings`と同種の位置づけ）
...
オプトインです。並び順は `obj=` の値ごとに定義されており（`building`/`vehicle`/`way`/`good`/`bridge`/`tunnel`/`roadsign`/
...
`src/simutrans/simfab.cc`）を根拠とする「静的解析」層のルールで、`couplings`
（車両連結制約の充足可能性）と同種の位置づけです。makeobj 自体はこの種の問題を一切検証しません。
ランタイムコードは `descriptor/writer/` と異なり網羅的に列挙できる構造ではないため、この種のルールは
「宣言データだけで閉じた問題」（単一の `.dat` ファイル内で完結する、または `couplings` のように
1ディレクトリ内で完結する問題）に限定して追加しています（詳細は下記「既知の制限」参照）。
...
- field の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  ...
  `obj=field`を参照しているかの横断検証は、1ファイル単位の`lint`ではなく`couplings`
...
- `fmt --reorder` でのコメント保持は、直後の`Pair`行に紐づく場合のみ対応しています
  （詳細は上記「`--reorder` とコメント行の扱い」参照）。紐づけ先が無い孤立コメント
...
```
src/
  main.rs                clap による CLI 入口（lint/fmt/couplings。lintは複数ファイル
                         （ディレクトリ再帰・globパターン）・--configに対応。
                         --helpの短い説明文はconfigのlanguageに応じて動的に翻訳する）
  config.rs               lint/fmt/couplings共通設定（TOML）。[rules]診断ルールinclude/exclude
                         （Diagnostic.code単位）と[general] languageの2セクション。
