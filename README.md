...
（`building_writer.cc` / `vehicle_writer.cc` / `get_waytype.cc` / `image_writer.cc` /
...
### `lint` — 静的検証（`obj=building` / `obj=vehicle`）
...
並び順は `obj=` の値ごとに定義されており（`building`/`vehicle` に対応）、未対応の obj 種別では
...
`src/rules/vehicle.rs` 冒頭コメント参照）。building のルールは vanilla Simutrans と
OTRP（Simutrans-Extended 系フォーク）の該当ファイルを diff し、両者で一致することも
確認済みです。vehicle のルールは vanilla Simutrans のみで確認済みで、OTRP との個別 diff は
まだ行っていません。

対応範囲は現状:

- `lint`: `obj=building`（`type=extension` / `stop` / `depot` 系）、`obj=vehicle`
- `couplings`: `obj=vehicle` の `constraint[prev]` / `constraint[next]`

### 既知の制限（意図的に非対応）

- `obj=way` など building/vehicle 以外の obj 種別
