...
`factory_writer.cc` / `sound_writer.cc` / `ground_writer.cc` /
...
citycar/pedestrian/factory/sound/ground/menu/cursor/symbol/smoke/field/misc の全22 obj種別をカバーしています。building〜factoryの
14種はこのプロジェクトが事前に合意した対応範囲で、sound・ground・menu・cursor・symbol・smoke・field・miscはその計画完了後に追加された
対象範囲です（詳細は下記sound・ground・menu・cursor・symbol・smoke・field・miscの節を参照）。soundマイルストーンでの
`descriptor/writer/`配下の機械的な棚卸しにより、`ground`・`menu`・`cursor`・`symbol`・`smoke`・`field`・`misc`という計7種の
新規obj種別が発見されており、本マイルストーンはそのうち`misc`を対象にした7件目（最後）です
（`menu`/`cursor`/`symbol`/`smoke`/`field`/`misc`の6種は共通の基底クラス`skin_writer_t`の
サブクラスで、`write_obj`ロジック自体は全種で共有されている。詳細は`src/rules/menu.rs`/
`src/rules/cursor.rs`/`src/rules/symbol.rs`/`src/rules/smoke.rs`/`src/rules/field.rs`/
`src/rules/misc.rs`冒頭コメント参照）。misc対応をもって、`descriptor/writer/`配下の
全24ヘッダファイルを対象とした`register_writer(true)`/`get_type_name()`の機械的な棚卸しにより、
makeobjが認識するトップレベルobj種別（22種）を本ツールが完全にカバーしたことを確認しています
（詳細は下記「obj種別カバレッジの完全性について」の節を参照）。
...
| 静的解析 | `analyze --kind coupling` | 実行しなくても分かるゲーム内利用時の問題を検知 | PHPStan |

`lint` は1ファイル単位の検証、`analyze --kind coupling` は1ディレクトリ内の複数 `obj=vehicle` を
横断するグラフ解析（連結制約の充足可能性）です。スコープが異なるため意図的に別サブコマンドとして
分離しています（`analyze`は`--kind`で解析種別を選択する設計のため、将来別の横断解析を追加する
場合もサブコマンド自体は増えず`--kind`のバリアントが増える形になります）。
...
指定する必要があります（サブコマンドを省略して `dat_linter <file>` とする旧来の
ショートカットは廃止しました）。
...
### `lint` — 静的検証（`obj=building` / `obj=vehicle` / `obj=way` / `obj=good` / `obj=bridge` / `obj=tunnel` / `obj=roadsign` / `obj=crossing` / `obj=way-object` / `obj=ground_obj` / `obj=tree` / `obj=citycar` / `obj=pedestrian` / `obj=factory` / `obj=sound` / `obj=ground` / `obj=menu` / `obj=cursor` / `obj=symbol` / `obj=smoke` / `obj=field` / `obj=misc`）
...
自身が glob パターンを解釈します（`glob` クレート使用）。glob 展開・ディレクトリ再帰の
どちらの経路でも、収集した `.dat` ファイル一覧はパスの辞書順で処理します。
...
1ファイルでも error または warning が1件でもあれば、全体の終了コードは失敗（`1`）に
なります（単一ファイルを指定した場合は従来通りその1ファイルの結果のみを終了コードに反映し、
集計行も追加しません）。
...
2. 候補集合から `exclude` に列挙された `code` をさらに除外する（`exclude` は常に `include`
   より優先。同じ `code` が両方にあれば無効になる）。

```
dat_linter lint --config dat_linter.toml path/to/file.dat
```

**この include/exclude は `lint` だけでなく `fmt`（`parse_entries`/`format_reordered`が
出すwarning）・`analyze`（`couplings.rs`が出すDiagnostic）にも全く同じ意味論で適用されます**
（第9弾で対応。`fmt`のwarningには`fmt-leading-space-line`/`fmt-malformed-line`/
`fmt-reorder-unsupported-obj`/`fmt-reorder-lines-dropped`の4つのcodeが、`analyze`には
既存の`read-dir-failed`/`read-failed`/`missing-name`/`dangling-vehicle-constraint`/
`unsatisfiable-constraint`の5つのcodeが付与されています）。デフォルト（`--config`未指定・
include/exclude空）は3サブコマンドとも「全code許可」で変わりません。
...
省略時は全件表示）。一覧は `src/codes.rs::ALL_CODES` から取得され、実際のソースとの
整合性は `tests/codes_completeness.rs` が保証しています（ルールの追加・削除時に
一覧の更新を忘れるとテストが落ちます）。`--config` を指定すると、各 `code` が
現在の設定で有効(`enabled`)か無効(`disabled`)かも併記されます。
...
`dat_linter list` で確認した `code` について、「なぜ検出されるのか」（makeobj/ゲーム
ランタイムの実際の挙動を根拠とする）と「どう直せばよいか」を1件ずつ表示します。
説明文は `src/codes.rs::ALL_CODES` の各エントリに保持されており、`--config` の
`[general] language` に応じてJA/ENが切り替わります（他のサブコマンドと同じ言語設定を
共有します）。存在しない `code` を指定した場合は `dat_linter list` への案内を添えて
エラー終了します。
...
カレントディレクトリに自動生成します（`--help` 表示時も含め、起動のたびに探索されるタイミングで
生成されます）。生成に失敗しても（書き込み権限が無い等）致命的エラーにはせず、
設定ファイル無しの状態のまま動作を継続します。生成される内容は次の「出力言語」節・
「`fmt` の並び替えデフォルト設定」節に記載の値と `[rules] include/exclude` の空配列のみです。

#### 出力言語

設定ファイルの `[general]` セクションで、診断メッセージ・サマリ行・`--help` の短い説明文
（後述）の出力言語を切り替えられます。
...
（このツールの実装過程では日本語メッセージが先に実装されましたが、最終的にデフォルト言語は
英語に決定しています。既存の日本語固定の挙動を期待しているスクリプト等がある場合は、
明示的に `language = "ja"` を設定してください）。不正な値（`"en"`/`"ja"` 以外）が指定された
場合も英語にフォールバックします。

`lint`/`fmt`/`analyze`/`list` の全サブコマンドが `--config`（またはカレントディレクトリの
`dat_linter.toml` 自動探索）でこの設定を共有します。翻訳対象は診断メッセージ本文
（`missing-tile-image` 等、全79箇所）と各サブコマンドのサマリ・ステータス文言
（「OK（既知ルールの範囲では問題なし）」等、約20箇所）、各サブコマンドの短い1行説明
（`about`）、そして**各引数のhelp文字列**（`<PATH>`/`-v`/`--config`/`--no-reorder`/
`--write`/`<DIR>`/`--kind`/`--source`。第9弾で対応。それまでは`analyze`新設時に
この仕組みへ追随できておらず日本語ハードコードのまま残っていた）です。
**`lint --help` が表示する22obj種別の長い一覧文は翻訳対象外で、常に日本語の
まま表示されます**（`Diagnostic.code` のような識別子・`obj=building` のような`.dat`記法自体は
どちらの言語でも変わりません）。

```
dat_linter lint --config dat_linter.toml path/to/file.dat   # 設定ファイルのlanguageに従う
dat_linter --help                                             # 短い説明文もlanguageに従う
dat_linter analyze --help                                     # 引数helpもlanguageに従う（第9弾）
```

#### `fmt` の並び替えデフォルト設定

`fmt` が並び替え（慣習的な順序への reorder）を行うかどうかは、専用の設定項目ではなく
**`[rules] include/exclude`（既存の code 単位の仕組み）で制御します**。reorder自体は
`fmt-reorder-applied` という code（`Diagnostic::info`。`fmt`が実際に並び替えを適用した
タイミングで発行される）で表現されており、この code を `exclude` に加えると恒久的に
無効化できます。

```toml
# dat_linter.toml
[rules]
exclude = ["fmt-reorder-applied"]  # fmtの並び替えを恒久的に無効化する
```

**デフォルト（`[rules] exclude` に `fmt-reorder-applied` が無い場合）は並び替えを行います**
（他の code と同じ「exclude に無ければ有効」という意味論がそのまま適用されるため、
専用の真偽値フィールドは不要になりました）。CLIの `--no-reorder` フラグは、この
config設定に関わらず**そのプロセスの実行に限り**強制的に無効化します
（優先順位: `--no-reorder`指定 > config の `[rules] exclude`）。

（旧: 以前は専用の `[fmt] reorder = true/false` という設定セクションがありましたが、
第11弾でこの `[rules] include/exclude` 方式に統合し、`[fmt]` セクション自体を廃止しました。
既存の設定ファイルに残っている `[fmt]` セクションは、serde/tomlのデフォルトの
unknown-field許容仕様により単に無視されます — パースエラーにはなりません。ただし
意味を持たなくなった記述のため、削除するか上記の `[rules] exclude` 方式に書き換えることを
推奨します。）

**building** で検出する主な項目（すべて makeobj ソースで裏付け済み）:

- `cursor` と `icon` が両方未指定 → `type=stop`/`extension`/`depot`/`habour`/`harbour`/`dock`/`hq`
  ではビルドメニューに表示されない（makeobj はエラーを出さない）。ただし
  `type=res`/`com`/`ind`/`cur`/`mon`/`tow`ではこの根拠が当てはまらないため info
  （後述の「調査済み: `missing-cursor-icon`」参照）
- タイルに front/back image が1枚もない → 空画像タイルが黙って生成される
  （`"-"`＝画像なしセンチネルが指定されているタイルは対象外。`"-"`は
  `image_writer_t::write_obj`が空文字列と同様に無条件で「画像なし」として扱う値で、
  ファイル名として解決しようとはしません。複数タイルのbuilding/factoryで
  一部タイルだけを意図的に空にする実例（pak128 `factories/cotton_farm_w_fields.dat`
  の`BackImage[...]=-`等）で確認済みです。この判定は`check_image_ref`
  （`src/rules/common.rs`）側に一元化されています。詳細は後述の
  「調査済み: `"-"`センチネルの一元化」参照）
...
> **調査済み: `type=station` / `extension_building=1` は誤検知ではない**。
> `refs/linter_test/JpClassicTerminal.dat` はこの2つのobsolete構文を使っており、
> 一見古いpak128.japan系アドオンが単に動いていたように見えるが、現行の makeobj
> （`building_writer.cc:185-204`）はこれらを`dbg->fatal(...)`で実際に弾く。つまり
> このファイルは**現在のmakeobjではpak化に失敗する**のが正しい挙動であり、
> `obsolete-type`/`obsolete-keyword`両方を検出するこのツールの判定は調整不要
> （回帰テストは`tests/real_data_regression.rs`参照）。

> **調査済み: `missing-cursor-icon`はres/com/ind/cur/mon/tow・factoryでは対象外**。
> pak128実データ全体（2737ファイル）の`lint`で`missing-cursor-icon`が1368件と
> 全ルール中最多だったため、`builder/hausbauer.cc`を根拠に再調査した。
>
> - プレイヤーが実際に選んでビルドできる建物メニューを生成する唯一の経路
>   `hausbauer_t::fill_menu()`（284-319行目）は、`hausbauer_t::successfully_loaded()`
>   （140-212行目）が`station_building`リストへ登録した5種類の`btype`
>   （`.dat`の`type=`では`habour`/`harbour`/`dock`/`depot`/`stop`/`extension`）
>   だけを対象にする。この5種は`register_desc()`（234-236行目）で`cursor`が
>   実在する場合のみビルドツールが生成されるため、`missing-cursor-icon`の
>   根拠（cursor/icon省略→ビルドメニュー非表示）はこれらの`type`には正しく
>   当てはまる
> - `res`/`com`/`ind`（`city_res`/`city_com`/`city_ind`）は`city_residential`/
>   `city_commercial`/`city_industry`という**別の**リストへ登録され、
>   `get_residential()`/`get_commercial()`/`get_industrial()`経由で都市成長
>   シミュレーション（`world/simcity.cc`）が自動選択・自動配置する。選定条件
>   （`is_allowed_climate`/`distribution_weight`/`is_available`/サイズ）に
>   cursorへの言及は一切無く、プレイヤーが選ぶビルドメニューにも一切現れない
> - `mon`/`cur`/`tow`も同様に`monuments`/`attractions_land`/`attractions_city`/
>   `townhalls`という別リストへ登録されるのみ。`monuments`は`world/simcity.cc`
>   側の特殊建造物自動配置ロジック（`get_special()`）が使うだけで、
>   `attractions_land`/`attractions_city`/`townhalls`を読むのは
>   `gui/curiosity_edit.cc`という**pakset編集者向けツール**のみ（通常のプレイヤー
>   向けビルドメニューではない）
> - `hq`は`register_desc()`が`tool_headquarter_t`を個別に生成する特殊経路を
>   持ち、`city_res`等とは異なり実際にcursorの有無がプレイヤーの建設可否に
>   影響するため対象外リストには含めない
> - `obj=factory`（`building_desc_t::factory`）は`successfully_loaded()`の
>   switch文で`case building_desc_t::factory: break;`となっており、
>   `station_building`を含むどのリストにも登録されない。実際の配置は
>   `builder/fabrikbauer.cc`（cursorへの言及が一切無い、別モジュール）が
>   都市の産業需要等に応じて行う
>
> ユーザーからの当初の仮説（`NoConstruction=1`が関係するのではないか）は
> 実データで検証した結果誤りだった（pak128 `cityhouses/`配下1187ファイル中
> `NoConstruction`使用は0件）。正しい判別軸は`NoConstruction`ではなく`type=`の値。
>
> pak128実データで実際に`missing-cursor-icon`が出ていた1368件を`type=`別に
> 集計すると、`res`858 / `com`173 / `ind`158 / `cur`63 / `mon`18 / `tow`2 /
> `type=`未指定（`obj=factory`）96で合計1368と過不足なく一致し、
> `stop`/`extension`/`depot`/`dock`/`hq`の実例は1件も含まれていなかった。
> このため`src/rules/building.rs`の`TYPES_WITHOUT_BUILD_MENU`（`res`/`com`/
> `ind`/`cur`/`mon`/`tow`）と`src/rules/factory.rs`のfactory向け分岐で、
> これらの種別のみ`missing-cursor-icon`（error）ではなく
> `cursor-icon-not-applicable`（info）を出すよう変更した
> （回帰テストは`tests/building.rs::cursor_icon_not_applicable_for_res_com_ind_cur_mon_tow`
> と`tests/factory_lint.rs::cursor_icon_not_applicable_for_factory`参照）。
...
- `engine_type` が既知値（`diesel`/`electric`/`steam`/`bio`/`sail`/`fuel_cell`/`hydrogene`/`battery`/`unknown`）
  ...
  **`engine_type=none`も既知値ではありません**（`vehicle_desc_t::engine_t`列挙型・
  `get_engine_type()`のどちらにも`none`は存在しない）。pak128の無動力車両
  （貨車等）で`engine_type=none`という記述が161件見つかっていますが、これは
  linterの誤検知ではなく、実際に`diesel`へ静かにフォールバックする本物の問題です
  （`"none"`は`waytype`/`constraint[prev/next]`では正当な特殊値ですが、
  `engine_type`にこの慣習を誤って転用したものと考えられます。詳細は
  `src/rules/vehicle.rs`の`KNOWN_ENGINE_TYPES`冒頭のREJECTEDコメント参照）
...
- `power` を宣言しているのに `gear` が変換後 `(gear*64/100)` で 0 になる（`gear=0` または `gear=1`）
  → WARNING。編成内でのこの車両の実効出力寄与が常に0になります（`simconvoi.cc`:
  `sum_gear_and_power += get_power() * get_gear()`）。**このルールのみ他と根拠の種類が
  異なります**: makeobj自体（コンパイル時）ではなく、ゲームエンジンのランタイムコード
  （`simconvoi.cc`）を根拠とする「静的解析」層のルールです（`analyze --kind coupling`と同種の位置づけ）
...
- `openimage[ns][0]` / `openimage[ew][0]`（`make_list`による`[0][1][2]...`連番走査）が
  ...
  実データでは`OpenImage[NS,EW][0-1]=...`のような方向名（ribi）文字列パラメータ展開
  （`src/param_expansion.rs`参照）でこれらのキーを一括定義する記法もよく使われる
...
- 画像キー（`{front|back}image[{ribi}]`26方向×2 / `{front|back}imageup[{slope}]`
  ...
  crossingと同じ`check_image_ref`を共有）。ただし`"-"`（画像なしセンチネル）が
  指定されている場合はファイル名として解決しようとしない
  （詳細は後述の「調査済み: `"-"`センチネルの一元化」参照）
...
> **調査済み: `"-"`センチネルの一元化（第8弾）**。ユーザーから
> `iss/way-object/road/wall_1.dat`の`backdiagonal[nw]=-`が`missing-image-file`
> として誤検知される報告があった。第6弾（項目2）ではbuilding/factoryのタイル画像
> ルールに`v != "-"`ガードを個別追加していたが、`check_image_ref`
> （`src/rules/common.rs`）自体は`"-"`を判定していなかったため、これを呼び出す
> 他の呼び出し元（bridge/citycar/crossing/groundobj/pedestrian/roadsign/tree/
> way_obj/way等の大半、building/factory/ground/tunnelの一部を除く21箇所）は
> `"-"`をファイル名として解決しようとし、同種の誤検知を起こし得る状態だった。
>
> `image_writer_t::write_obj`（image_writer.cc:348-453）を再確認した結果、
> 366行目の`if (imagekey != "-" && imagekey != "")`がファイル解決ロジック全体
> （row/col解析・パス構築・`block_load`によるファイル読み込み、行370-450）を
> 包んでおり、`"-"`または空文字列の場合はファイル名として解決しようとせず、
> 451行目の`else`分岐で空画像ノードを書き込むのみ（fatal/errorにはならない）
> ことを確認した。この関数は**全obj種別が画像参照を書き込む際に必ず経由する
> 共有関数**であり、`"-"`の特別扱いはper-obj-type固有の挙動ではなく
> `image_writer_t::write_obj`という共有経路そのものの性質である。
>
> このため、`"-"`の判定を`check_image_ref`の入口一箇所に一元化した
> （`cursor-icon-not-applicable`と同様、無言にはせずinfo診断
> `image-ref-empty-sentinel`を出す）。building/factory/ground/tunnelに
> あった個別の`v != "-"`ガードは冗長になったため削除し、`check_image_ref`
> 呼び出し側は`"-"`を意識せず値をそのまま渡せばよい設計にした
> （既存のbuilding/factory向け回帰テストは全てそのまま green を確認済み。
> way-objectの新規回帰テストは`tests/way_obj_lint.rs::dash_sentinel_in_backdiagonal_is_not_a_false_positive`）。
>
> pak128実データ全体で`"-"`が使われる他の箇所についても横断確認したが、
> 本件の`check_image_ref`一元化により全obj種別で一貫して対応済みとなった
> （個別obj種別ごとの追加対応は不要）。
...
- citycarは`obj=vehicle`と異なり`waytype`/`engine_type`/`freight`/
  ...
  概念を持たない自動生成NPC車両のため、vehicleの`analyze --kind coupling`が対象とする
...
- factoryは`factory_writer_t::write_obj`内で**`building_writer_t::write_obj`を
  ...
  1枚も無いタイルは、factoryの`.dat`にもそのまま同じ形式・同じ重大度で適用される
  （`"-"`＝画像なしセンチネルの扱いもbuildingと共通。複数タイルのfactoryで
  一部タイルだけを意図的に空にする実例（pak128 `factories/cotton_farm_w_fields.dat`）
  で確認済み）。ただし`cursor`/`icon`両方未指定については、buildingと**同じ重大度
  ではなくfactory向けに専用の判断をする**（次項参照）
- `cursor`/`icon`両方未指定 → `hausbauer_t::successfully_loaded()`
  （`case building_desc_t::factory: break;`）がfactoryをプレイヤー向けビルドメニュー
  （`fill_menu()`が読む`station_building`リスト等）へ一切登録しないため、
  factoryはビルドメニュー非表示という根拠自体が当てはまらない。実際の配置は
  `builder/fabrikbauer.cc`（cursorへの言及が無い別モジュール）が行う。pak128実データ
  でも全factoryが`cursor`/`icon`未指定のまま流通していることを確認済み（詳細は
  building節の「調査済み: `missing-cursor-icon`」参照）。そのため`missing-cursor-icon`
  （error）ではなく`cursor-icon-not-applicable`（info）を出す
...
- **[静的解析]** `productivity=0` → makeobj自体はノーチェックで通すが、ゲーム
  ランタイム（`simfab.cc`）の`fabrik_t`コンストラクタがfactory配置時に無条件で
  呼ぶ`update_scaled_pax_demand()`/`update_scaled_mail_demand()`が
  `productivity`を分母とした整数除算を行うため、配置した瞬間にゼロ除算
  （未定義動作、通常はクラッシュ）が発生する。根拠はmakeobjのコンパイル時
  ソースではなくゲームランタイムコードで、vehicleの`power-gear-mismatch`と
  同じ「静的解析」層のルール（`factory-productivity-zero`, ERROR）
...
**ground**（水面・傾斜地・気候別テクスチャなど地面タイルそのものの画像定義。`.dat`に実際に
書く`obj=`の値は`ground`。既存の`ground_obj`（岩・草むら等の地面装飾オブジェクト、
`groundobj_writer.cc`）とは別のwriter・別のobj種別。事前合意の14種計画には含まれておらず、
soundマイルストーンでの`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別
のうち最初に対応した1件）で検出する主な項目（`ground_writer.cc` / `imagelist2d_writer.cc` /
`imagelist_writer.cc` / `image_writer.cc`を精読して確認）:

- `image[<slope>][<phase>]`（slope=0..127、phaseは可変）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。`ground_writer.cc`は
  good/soundと同様に`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、
  かつslope=0のphase0が欠落した時点でmakeobj自身が即座に走査を打ち切るだけでFATALにしない
  ため（`src/rules/ground.rs`冒頭のREJECTEDコメントに詳細を記録）
- `climates`/`waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません。気候・傾斜の
  意味づけは全てslope番号の**位置**で決まる実行時ロジック（`ground_desc_t`側）であり、
  .dat記述側は純粋な位置引数の画像リストを書くだけです
- `"-"`（画像なしセンチネル）はphase走査を止めません（`obj.get()`が返す空文字列とは異なる）。
  実際のpak128実例（`landscape/grounds/slope.dat`）でも`Image[0][0]=-`という書き方で
  途中slopeを意図的に空けています

**menu**（ビルドメニューのウィンドウ枠・ボタン等、UIスキン画像定義。`.dat`に実際に書く`obj=`の
値は`menu`。事前合意の14種計画には含まれておらず、soundマイルストーンでの
`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別のうち2件目に対応した
1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` / `imagelist_writer.cc` /
`image_writer.cc`を精読して確認）:

- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。`skin_writer_t::write_obj`
  （`menuskin_writer_t`はこれをオーバーライドしない）はgood/sound/groundと同様に
  `write_name_and_copyright`を無条件フォールバックで呼ぶだけで、かつ`image[0]`が欠落した時点で
  makeobj自身が即座に走査を打ち切るだけでFATALにしないため
  （`src/rules/menu.rs`冒頭のREJECTEDコメントに詳細を記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- **`"> "`（ズーム不可フラグ）構文の発見**: `image_writer_t::write_obj`は値の先頭に`'>'`が
  あればこれを「ズーム不可」フラグとして剥がし、`'>'`の有無に関わらず**無条件で**前後の空白を
  トリムしてから画像を解決します（image_writer.cc:356-364）。実際の公開pak
  （`aburch/simutrans-pak128.britain:gui/gui64/skins-64.dat`の`Image[0]=> skins.0.4`等）で
  この構文が使われていることを確認しました。この処理はobj種別中立の共通ロジックのため、
  `common::check_image_ref`側に`strip_zoomable_prefix_and_trim`として修正を反映しています
  （修正前は`"> skins.0.4"`のような値をそのままファイル名として解決しようとし、実在する
  ファイルを「見つからない」と誤検知していました。この調査の過程で、`trim()`が`'>'`の
  有無に関係なく常に呼ばれる実装であることも判明したため、既存の`building`テスト
  （先頭スペースを含む`cursor=`値がエラーになるという主張）も、実際のmakeobj挙動
  （先頭スペースはトリムされて正常に解決される）に合わせて訂正しました）
- **ディレクトリを跨ぐ相対パス参照（`../`）の修正（第10弾）**: `image_writer_t::write_obj`
  （image_writer.cc:372-388）のファイル名解決は実際には2段階です:
  1. `imagekey.rfind('/')`で値全体から**最後の`'/'`**を探し、それより後ろの部分
     （ディレクトリ接頭辞を除いたベース名+行列番号部分）を`numkey`とする
  2. `numkey`（ディレクトリ接頭辞を除いた部分）の中で最初の`'.'`を探し、そこを基準に
     元の値全体（ディレクトリ接頭辞を含む）を切り詰めて`.png`を付与する

  以前の実装は手順1を欠いており、**値全体**の中で最初の`'.'`を探していました。
  そのため`icon=> ../../icon_way3.1.0`のような相対パス参照では、`../../`の`..`自体に
  含まれる`'.'`に誤ってヒットし、ファイル名が空文字列（`.png`のみ）に解決されてしまい、
  実在するファイルを「見つからない」と誤検知していました（ユーザー報告:
  `refs/linter_test/iss/building/depot/depot.dat`の`icon=> ../../icon_way3.1.0`が
  `Referenced image .png was not found`として誤検知）。正しくは`numkey`="icon_way3.1.0"
  （最後の`/`より後ろ）の中で最初の`.`を探し、`../../icon_way3.png`に解決されるべきです。
  **makeobjは`rfind('/')`のみを見ており`'\'`（バックスラッシュ）は区切り文字として
  扱いません**（Windows的な直感に反しますが、実データが`/`区切りで書かれている以上
  makeobjに忠実に揃えています）。`common::resolve_image_filename`にこのアルゴリズムを
  正確に反映し、回帰テストを`src/rules/common.rs`（単体テスト5件）と`tests/building.rs`
  （実際の`iss/building/depot/depot.dat`相当のディレクトリ構造を再現した統合テスト
  `relative_path_with_double_dot_in_directory_prefix_is_not_a_false_positive`）に
  追加しました。

**cursor**（マウスカーソルのスキン画像定義。`.dat`に実際に書く`obj=`の値は`cursor`。
building/way/bridge等多くのobj種別が持つ`cursor=`/`icon=`**フィールド**とは全くの別概念
（トップレベルのobj種別）であり注意が必要。事前合意の14種計画には含まれておらず、
soundマイルストーンでの`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別
のうち3件目に対応した1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` /
`imagelist_writer.cc` / `image_writer.cc`を精読して確認）:

- `cursorskin_writer_t`（skin_writer.h）は`menuskin_writer_t`と同じく共通の基底クラス
  `skin_writer_t`のサブクラスで、`get_type()`/`get_type_name()`の2つのオーバーライドのみを持ち
  `write_obj`は一切オーバーライドしない。実際にskin_writer.h/skin_writer.ccを読み直して独立に
  確認した結果、`cursor`のロジックは**`menu`と挙動上完全に同一**であることを確認済み
  （詳細は`src/rules/cursor.rs`冒頭コメント参照）
- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。menuと全く同じ理由
  （`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、かつ`image[0]`が欠落した時点で
  makeobj自身が即座に走査を打ち切るだけでFATALにしないため。詳細は`src/rules/cursor.rs`冒頭の
  REJECTEDコメントに記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- `"> "`（ズーム不可フラグ）構文は実際の公開pak（`simutrans/pak128:base/misc_GUI/mouse.dat`の
  `Image[0]=> mouse.1.0`）でcursorスキンにも使われていることを確認しました。この処理は
  menuマイルストーンで`common::check_image_ref`側（obj種別中立の共通ロジック）に既に実装済みの
  `strip_zoomable_prefix_and_trim`がそのまま適用されるため、cursor固有の追加対応は不要でした

**symbol**（ビルドメニューのシンボルアイコン・ロゴ等、UIスキン画像定義。`.dat`に実際に書く`obj=`の
値は`symbol`。事前合意の14種計画には含まれておらず、soundマイルストーンでの
`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別のうち4件目に対応した
1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` / `imagelist_writer.cc` /
`image_writer.cc`を精読して確認）:

- `symbolskin_writer_t`（skin_writer.h）は`menuskin_writer_t`/`cursorskin_writer_t`と同じく
  共通の基底クラス`skin_writer_t`のサブクラスで、`get_type()`/`get_type_name()`の2つの
  オーバーライドのみを持ち`write_obj`は一切オーバーライドしない。実際にskin_writer.h/
  skin_writer.ccを読み直して独立に確認した結果、`symbol`のロジックは**`menu`/`cursor`と
  挙動上完全に同一**であることを確認済み（詳細は`src/rules/symbol.rs`冒頭コメント参照）
- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。menu/cursorと全く同じ理由
  （`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、かつ`image[0]`が欠落した時点で
  makeobj自身が即座に走査を打ち切るだけでFATALにしないため。詳細は`src/rules/symbol.rs`冒頭の
  REJECTEDコメントに記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- `"> "`（ズーム不可フラグ）構文は、menuマイルストーンで`common::check_image_ref`側
  （obj種別中立の共通ロジック）に既に実装済みの`strip_zoomable_prefix_and_trim`がそのまま
  適用されるため、symbol固有の追加対応は不要でした。実際の公開`.dat`
  （`simutrans/pak128:base/misc_GUI/builder_symbol.dat`の`Image[0]=builder_symbol.1.0`等）で
  `obj=symbol`が使われていることをGitHub code searchで確認済みです

**smoke**（車両・工場が排出する煙エフェクトのスキン画像定義。連番の`image[N]`がアニメーション
フレーム列を表す。`.dat`に実際に書く`obj=`の値は`smoke`。事前合意の14種計画には含まれておらず、
soundマイルストーンでの`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別のうち
5件目に対応した1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` / `imagelist_writer.cc` /
`image_writer.cc`を精読して確認）:

- `smoke_writer_t`（skin_writer.h）は`menuskin_writer_t`/`cursorskin_writer_t`/
  `symbolskin_writer_t`と同じく共通の基底クラス`skin_writer_t`のサブクラスで、
  `get_type()`/`get_type_name()`の2つのオーバーライドのみを持ち`write_obj`は一切
  オーバーライドしない。「smoke」という名前が視覚エフェクトのタイミング処理（アニメーション周期・
  フェード等）を連想させたため、他3種の結論を鵜呑みにせずskin_writer.h/skin_writer.ccを実際に
  読み直して独立に確認したが、そのような特別な処理は`write_obj`側に一切無く、
  `menu`/`cursor`/`symbol`と**挙動上完全に同一**であることを確認した（詳細は
  `src/rules/smoke.rs`冒頭コメント参照）。アニメーション周期・フェード相当の処理
  （`smokeuplift`/`smokelifetime`）は`obj=factory`側のフィールドとゲーム実行時ロジックに
  存在し、この`obj=smoke`自体は単に連番画像を保持するスキン画像リストに過ぎない
- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。menu/cursor/symbolと
  全く同じ理由（`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、かつ`image[0]`が
  欠落した時点でmakeobj自身が即座に走査を打ち切るだけでFATALにしないため。詳細は
  `src/rules/smoke.rs`冒頭のREJECTEDコメントに記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- `"> "`（ズーム不可フラグ）構文は、menuマイルストーンで`common::check_image_ref`側
  （obj種別中立の共通ロジック）に既に実装済みの`strip_zoomable_prefix_and_trim`がそのまま
  適用されるため、smoke固有の追加対応は不要でした。実際の公開`.dat`
  （`simutrans/pak128:base/smokes/Smokes.dat`の`Image[0]=misc-smoke-128.0.0`等、5フェーズの
  連番画像を持つ実例）で`obj=smoke`が使われていることをGitHub code searchで確認済みです。
  なお`obj=factory`が持つ`smoketile[N]=`/`smokeoffset[N]=`/`smoke=`**フィールド**（工場の
  どのタイル・座標からこの`obj=smoke`を参照するかの指定。`src/rules/factory.rs`の
  `SmokeOffsetRule`が対象）とは全くの別概念であり、混同しないこと

**field**（作物の生育段階（収穫フェーズ）ごとのスキン画像定義。`obj=factory`の`fields=`フィールドが
名前で参照する。`.dat`に実際に書く`obj=`の値は`field`。事前合意の14種計画には含まれておらず、
soundマイルストーンでの`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別のうち
6件目に対応した1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` / `imagelist_writer.cc` /
`image_writer.cc`を精読して確認）:

- `field_writer_t`（skin_writer.h）は`menuskin_writer_t`/`cursorskin_writer_t`/
  `symbolskin_writer_t`/`smoke_writer_t`と同じく共通の基底クラス`skin_writer_t`のサブクラスで、
  `get_type()`/`get_type_name()`の2つのオーバーライドのみを持ち`write_obj`は一切オーバーライドしない。
  「field」という名前が作物の生育段階の日数・タイミング処理を連想させたため、他4種の結論を
  鵜呑みにせずskin_writer.h/skin_writer.ccを実際に読み直して独立に確認したが、そのような特別な処理は
  `write_obj`側に一切無く、`menu`/`cursor`/`symbol`/`smoke`と**挙動上完全に同一**であることを確認した
  （詳細は`src/rules/field.rs`冒頭コメント参照）。生育段階の進行速度・タイミング相当の処理
  （`production_per_field`等）は`obj=factory`側のフィールドとゲーム実行時ロジックに存在し、
  この`obj=field`自体は単に収穫フェーズの連番画像を保持するスキン画像リストに過ぎない
- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。menu/cursor/symbol/smokeと
  全く同じ理由（`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、かつ`image[0]`が
  欠落した時点でmakeobj自身が即座に走査を打ち切るだけでFATALにしないため。詳細は
  `src/rules/field.rs`冒頭のREJECTEDコメントに記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- `"> "`（ズーム不可フラグ）構文は、menuマイルストーンで`common::check_image_ref`側
  （obj種別中立の共通ロジック）に既に実装済みの`strip_zoomable_prefix_and_trim`がそのまま
  適用されるため、field固有の追加対応は不要でした。実際の公開`.dat`
  （`simutrans/pak128:factories/corn_farm.dat`、`Obj=factory`ブロックと`obj=field`ブロックを
  1ファイルに`----`区切りで連結する構成。factory側の`fields=corn_field`がname一致で`obj=field`側の
  `name=corn_field`を参照する）で`obj=field`が使われていることを確認済みです。同様に
  `simutrans/pak128:factories/fields_agriculture.dat`は`obj=field`ブロックのみを複数連結した
  ファイルで、いずれも収穫段階5枚（うち1枚は積雪版）の`image[0..4]`という構成でした。
  なお`obj=factory`が持つ`fields=`/`max_fields=`/`min_fields=`/`start_fields=`**フィールド**
  （工場がこの`obj=field`を何個までどの名前で生成するかの指定）とは全くの別概念であり、
  混同しないこと

**misc**（ゲームがまだ「本物のオブジェクト」として統合していない、雑多なUI用画像を保持する
スキン画像定義。`skin_writer.h`の`miscimages_writer_t`直前のクラスコメント「Used for images
needed by the game but not yet integrated as real objects」より。`.dat`に実際に書く`obj=`の
値は`misc`。事前合意の14種計画には含まれておらず、soundマイルストーンでの
`descriptor/writer/`配下の機械的な棚卸しで発見された7種の新規obj種別のうち7件目（最後）に
対応した1件）で検出する主な項目（`skin_writer.cc` / `skin_writer.h` / `imagelist_writer.cc` /
`image_writer.cc`を精読して確認）:

- `miscimages_writer_t`（skin_writer.h）は`menuskin_writer_t`/`cursorskin_writer_t`/
  `symbolskin_writer_t`/`smoke_writer_t`/`field_writer_t`と同じく共通の基底クラス
  `skin_writer_t`のサブクラスで、`get_type()`/`get_type_name()`の2つのオーバーライドのみを持ち
  `write_obj`は一切オーバーライドしない。実際にskin_writer.h/skin_writer.ccを読み直して独立に
  確認した結果、`misc`のロジックは**`menu`/`cursor`/`symbol`/`smoke`/`field`と挙動上完全に同一**
  であることを確認済み（詳細は`src/rules/misc.rs`冒頭コメント参照）
- `image[<i>]`（i=0,1,2,...と1次元・無制限に走査）が実際に画像を指す場合（空文字列でも
  `"-"`（画像なしセンチネル）でもない場合）、参照画像が見つからない／サイズが128の倍数でない
  → FATAL ERROR（building等と同じ`check_image_ref`を共有）
- `name`/`copyright`未指定チェック、画像0枚チェックは追加していません。menu/cursor/symbol/
  smoke/fieldと全く同じ理由（`write_name_and_copyright`を無条件フォールバックで呼ぶだけで、
  かつ`image[0]`が欠落した時点でmakeobj自身が即座に走査を打ち切るだけでFATALにしないため。
  詳細は`src/rules/misc.rs`冒頭のREJECTEDコメントに記録）
- `waytype`/`cursor`/`icon`のような名前付きフィールドは存在しません
- `"-"`（画像なしセンチネル）は画像走査を止めません（`obj.get()`が返す空文字列とは異なる）
- `"> "`（ズーム不可フラグ）構文は、menuマイルストーンで`common::check_image_ref`側
  （obj種別中立の共通ロジック）に既に実装済みの`strip_zoomable_prefix_and_trim`がそのまま
  適用されるため、misc固有の追加対応は不要でした。実際の公開`.dat`
  （`simutrans/pak128:base/misc_GUI/construction.dat`の`Image[0]=construction.1.0`〜
  `Image[9]=construction.1.9`（10枚構成）、`aburch/simutrans-pak128.britain:gui/gui128/misc_images-128.dat`の
  `Image[0]`〜`Image[6]`（7枚構成））で`obj=misc`が使われていることをGitHub APIで直接fetchして
  確認済みです（GitHub code searchは部分一致ノイズが多かったため、有望なパス名を直接あたる
  方式で確認しました）

**obj種別カバレッジの完全性について**: misc対応をもって、vanilla simutransの pinned commit
`1d2799f9a73adf94751e2d8357fea9dabcc4f740`の`src/simutrans/descriptor/writer/`配下にある
**全24ヘッダファイル**（`bridge_writer.h`/`building_writer.h`/`citycar_writer.h`/
`crossing_writer.h`/`factory_writer.h`/`good_writer.h`/`ground_writer.h`/`groundobj_writer.h`/
`image_writer.h`/`imagelist_writer.h`/`imagelist2d_writer.h`/`obj_node.h`/
`obj_pak_exception.h`/`obj_writer.h`/`pedestrian_writer.h`/`roadsign_writer.h`/
`root_writer.h`/`skin_writer.h`/`sound_writer.h`/`text_writer.h`/`tree_writer.h`/
`tunnel_writer.h`/`vehicle_writer.h`/`way_obj_writer.h`/`way_writer.h`/`xref_writer.h`。
`*_writer.h`という命名規則に限定せず、ディレクトリ内の`.h`ファイル全てを対象とした）に対して、
`register_writer(true)`と`get_type_name()`の機械的な棚卸しを行いました。

`obj_writer_t::register_writer(bool main_obj)`（`obj_writer.cc:24-36`）の実装は
`if (main_obj) { writer_by_name->put(get_type_name(), this); }`であり、
`writer_by_name`こそが`obj_writer_t::write`（`obj_writer.cc:39-59`、`.dat`の`obj=`フィールドの
値でルックアップする実体）が引くハッシュテーブルです。つまり`register_writer(true)`で
登録されるクラスだけが「`.dat`に書ける`obj=`のトップレベル値」であり、`register_writer(false)`
（`tile_writer_t`/`factory_field_class_writer_t`/`factory_field_group_writer_t`/
`factory_smoke_writer_t`/`factory_product_writer_t`/`factory_supplier_writer_t`/
`image_writer_t`/`imagelist_writer_t`/`imagelist2d_writer_t`/`root_writer_t`/`text_writer_t`/
`xref_writer_t`）は`writer_by_type`（型IDでの内部参照専用）にのみ登録される内部ヘルパー
writerであり、`.dat`の`obj=`には書けないことも確認しました。

棚卸しの結果、`register_writer(true)`を持つクラスは厳密に**22個**であり、その
`get_type_name()`の返り値は次の22種で尽きます: `building`（`tile_writer_t`の`"tile"`は
`register_writer(false)`のため対象外）、`vehicle`、`way`、`good`、`bridge`、`tunnel`、
`roadsign`、`crossing`、`way-object`、`ground_obj`、`tree`、`citycar`、`pedestrian`、
`factory`、`sound`、`ground`、`menu`、`cursor`、`symbol`、`smoke`、`field`、`misc`。
これは`registry::RuleSet::for_obj_type`のmatch armと1対1で完全に一致しており（本misc追加で
22種目が埋まりました）、`lint`が対応するobj種別のカバレッジは**完全（22/22）**であると
判断しています。soundマイルストーンでの「これで最後」という過去の判断が実際には誤りだった
前例があるため断定はしませんが、少なくとも上記コミット時点のvanilla simutransソースに
限れば、機械的な全ヘッダ走査による裏付けのある結論です。将来的にmakeobj本体側に新規obj種別が
追加された場合は、改めてこの棚卸し手順（全`.h`ファイルへの`register_writer(true)`/
`get_type_name()`のgrep）を繰り返す必要があります。
...
`--no-reorder`（このプロセスの実行に限り、config設定に関わらずpreserve-order挙動にする）を
指定するか、`dat_linter.toml`の`[rules] exclude = ["fmt-reorder-applied"]`で恒久的に
無効化してください（優先順位: `--no-reorder`指定 > config設定。詳細は上記「`fmt` の並び替え
デフォルト設定」節を参照）。
並び順は `obj=` の値ごとに定義されており（`building`/`vehicle`/`way`/`good`/`bridge`/`tunnel`/`roadsign`/
`crossing`/`way-object`/`ground_obj`/`tree`/`citycar`/`pedestrian`/`factory`/`sound`/`ground`/`menu`/`cursor`/`symbol`/`smoke`/`field`/`misc` に対応）、未対応の obj 種別では並び替えを行わず元の順序のまま出力します。

`fmt`が出すwarning（行頭スペース行・`=`の無い不正行・未対応obj・並び替え時の
削除行数）にも、それぞれ`fmt-leading-space-line`/`fmt-malformed-line`/
`fmt-reorder-unsupported-obj`/`fmt-reorder-lines-dropped`というcodeが付与されており、
`lint`と同じ`--config`の`[rules] include/exclude`で個別に抑制できます（第9弾。
詳細は上記「設定ファイルによるルールのinclude/exclude」節参照）。

#### 複数ファイル一括処理

`fmt` の引数には単一ファイルパスの他に、`lint`と同じ`collect_dat_paths`
（ディレクトリ再帰走査・glob パターン）でディレクトリ・globパターンも指定できます。
**複数ファイルに解決された場合、`--write`（`-w`）を指定しないとエラー終了します**
（整形結果を複数ファイル分stdoutへ混在させて出すのは実用性が低いため）。`--write`を
指定した場合は各ファイルを整形して書き戻し、末尾に`lint`と同様の集計サマリ行が出力されます:

```
合計: 対象ファイル 4 件 / warning 3 件
```

#### 並び替えとコメント行の扱い

`#` 始まりのコメント行（1行以上連続可）は、間に空行を挟んでいてもそれをスキップして、
直後に現れる最初の `key=value` 行（`Pair`）に**紐づき、そのPairと一緒に並び替え後も移動します**。
例えばファイル先頭の著作権ヘッダーコメントは、通常 `obj=` 行（多くの obj 種別で並び順の先頭キー）に
紐づくため、結果的に並び替え後も出力の先頭に残ります。重複キー（同じキー名が複数回現れる場合）でも、
各コメントは安定ソートにより自分の直後にあった `Pair` とペアの関係を保ったまま出力されます。

以下のケースでは紐づけ先が無いと判断し、コメントは（他の行と同様）出力から削除されます
（削除した件数は警告として表示されます）:

- コメントの直後に `=` の無い不正行や行頭スペース行が来た場合（その行自体とコメントは道連れで削除）
- `-` 始まりの区切り行（複数 obj 定義の連結境界）をまたいで次のセグメントの Pair には紐づきません
- セグメント（またはファイル）の末尾までコメントの後に Pair が現れなかった場合（末尾コメント）

コメント以外の行（空行・行頭スペース行・`=` の無い不正行）は従来通り並び替え後の位置が
一意に決まらないため出力から除外され、件数が警告として表示されます。

### `analyze` — ディレクトリ横断解析

```
dat_linter analyze <path/to/vehicle_dat_dir>                   # --kind省略時はcoupling
dat_linter analyze <path/to/vehicle_dat_dir> --kind coupling   # 明示指定も可
```

1ディレクトリ内の複数 `.dat` を横断的に解析するサブコマンドです。`--kind`
（clap ValueEnum、現状は`coupling`の1種類のみ・デフォルトも`coupling`）で解析種別を選択します。
将来的に別の横断解析を追加する場合はこの列挙にバリアントを追加する設計にしており、
`registry::ObjType`と同じ「ワイルドカードarmを持たない網羅match」規約に従っています
（旧`couplings`サブコマンドはこの`analyze --kind coupling`に統合されました。正式公開前のため
`couplings`という名前の後方互換は提供していません）。

#### `--kind coupling` — 車両連結制約の静的解析
...
を検査します。`couplings.rs`が出す各`Diagnostic`（`read-dir-failed`/`read-failed`/
`missing-name`/`dangling-vehicle-constraint`/`unsatisfiable-constraint`）にも
`lint`と同じ`--config`の`[rules] include/exclude`が適用されます（第9弾）。
...
1件でもあれば `1`、指摘が1件も無ければ `0` になります（`fmt` の場合は
parse/format由来のwarning、`analyze --kind coupling` は現状 `error` のみを出しますが
同じ判定式です）。

### 出力チャンネル（stdout/stderr）

- **診断本文**（error/warn/info/debug、`[warn] code: message` のような1行）は
  `lint`/`fmt`/`analyze` いずれも **stderr** に出力されます。
- **情報メッセージ**（"OK" 相当の合格通知、件数サマリ行、`fmt` の書き込み成功
  メッセージ、`analyze` の読み込み件数）は **stdout** に出力されます。ただし
  指摘が1件も無い場合はこれらの情報メッセージ自体も省略されます（サイレント成功。
  上記「終了コード」参照）。
- `fmt` を `--write`（`-w`）無しで単一ファイルに対して実行した場合の整形結果本体
  （プログラムの主出力、ファイルへのリダイレクトを想定）は、上記の分類とは独立に
  常に stdout に出力されます。
...
`src/rules/sound.rs` / `src/rules/ground.rs` / `src/rules/menu.rs` / `src/rules/cursor.rs` /
`src/rules/symbol.rs` / `src/rules/smoke.rs` / `src/rules/field.rs` / `src/rules/misc.rs` 冒頭コメント参照）。
...
ground_obj・tree・citycar・pedestrian・factory・sound・ground・menu・cursor・symbol・smoke・field・misc のルールは vanilla Simutrans のみで確認済みで、OTRP との個別 diff はまだ行っていません。

大半のルールは makeobj 自体（コンパイル時、`descriptor/writer/`）の `dbg->fatal`/`dbg->warning` を
根拠としますが、`vehicle` の `power-gear-mismatch` と `factory` の `productivity-zero` の2ルールのみ
例外です。これらはゲームエンジンのランタイムコード（それぞれ `src/simutrans/simconvoi.cc` /
`src/simutrans/simfab.cc`）を根拠とする「静的解析」層のルールで、`analyze --kind coupling`
（車両連結制約の充足可能性）と同種の位置づけです。makeobj 自体はこの種の問題を一切検証しません。
ランタイムコードは `descriptor/writer/` と異なり網羅的に列挙できる構造ではないため、この種のルールは
「宣言データだけで閉じた問題」（単一の `.dat` ファイル内で完結する、または `analyze --kind coupling`
のように1ディレクトリ内で完結する問題）に限定して追加しています（詳細は下記「既知の制限」参照）。
21obj種別を横断調査した結果、この2件以外に同条件（単一.datで閉じて判定可能・ランタイム
ソースで裏付けが取れる・実害あり）を満たす候補は見つかっていません。
...
- `lint`: `obj=building`（`type=extension` / `stop` / `depot` 系）、`obj=vehicle`、`obj=way`、`obj=good`、
  ...
  `obj=citycar`、`obj=pedestrian`、`obj=factory`、`obj=sound`、`obj=ground`、`obj=menu`、`obj=cursor`、`obj=symbol`、`obj=smoke`、`obj=field`、`obj=misc`
- `analyze --kind coupling`: `obj=vehicle` の `constraint[prev]` / `constraint[next]`

このプロジェクトが事前に合意した対応obj種別計画（14種）と、計画完了後に追加された
sound・ground・menu・cursor・symbol・smoke・field・miscマイルストーン（15〜22種目）により、
makeobjが認識するトップレベルobj種別（22種、詳細は上記「obj種別カバレッジの完全性について」の
節を参照）を`lint`は完全にカバーしています。

### 既知の制限（意図的に非対応）

- pak64（またはpak128以外の`cell_size`）を対象としたアセットの画像サイズ検証。
  本ツールは`PAK_TILE_SIZE=128`固定で全ての画像参照を検証しますが、実際の
  makeobjの128倍数チェック（`image_writer.cc`の`img_size`）は`makeobj pak<N>`の
  CLI引数、または`.dat`内の`cell_size=`フィールドで決まる実行時の値です。
  同じpak128 submodule内に、意図的にpak64（デフォルトの`img_size`）でビルドされる
  補助アセット（例: pak128公式ソースの`base/misc_GUI_64/`、`Makefile`の
  `DIRS64`で`$(MAKEOBJ) PAK`が使われる。`DIRS128`は`$(MAKEOBJ) PAK128`）が
  同居していることがありますが、`.dat`ファイル単体にはこの区別を示す情報が
  （`cell_size=`が使われていない限り）無いため、本ツールから判別する手段が
  ありません（詳細は`src/rules/common.rs`の`PAK_TILE_SIZE`冒頭のREJECTED
  コメント参照）
- vehicle の `weight=0` 検出。`simconvoi.cc`で`sum_gesamtweight`による除算があるが
  `if(sum_gesamtweight != 0)`で明示的にガードされておりクラッシュしない（挙動が変わる
  だけで、makeobjの`dbg->fatal`/`dbg->warning`相当の明確な根拠が無い）ため対象外
- vehicle の `loading_time=0`（`cargo_max>0`との組み合わせ）検出。`simconvoi.cc`側で
  `+ 1`により除算がガードされ安全。かつ実際に問題が顕在化するかは「その車両が積み込みを
  実行する場面が来るか」という編成運用・ゲーム進行の状態に依存し、単一の`.dat`ファイルや
  1ディレクトリ内で閉じた判定ができないため対象外
- building/factory の `capacity=0` 系フィールド検出。意図的な0（機関車等の非貨物車両）と
  誤りの区別がmakeobjソース単体からは判別できず、実際に問題が顕在化するかはゲーム進行の
  状態に依存するため対象外
- ground の `name`/`copyright` 未指定検証、画像0枚（`image[0][0]`未指定）警告、および
  `image[<slope>][<phase>]`のslopeが範囲外（128以上）の場合の検出。いずれも
  `ground_writer.cc`側にfatal/warningの分岐が無い（詳細は`src/rules/ground.rs`冒頭の
  REJECTEDコメント参照）
- menu の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。いずれも`skin_writer.cc`側にfatal/warningの分岐が無い
  （詳細は`src/rules/menu.rs`冒頭のREJECTEDコメント参照）
- cursor の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。menuと全く同じ理由で`skin_writer.cc`側にfatal/warningの分岐が
  無い（`cursorskin_writer_t`は`write_obj`をオーバーライドしないため。詳細は
  `src/rules/cursor.rs`冒頭のREJECTEDコメント参照）
- symbol の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。menu/cursorと全く同じ理由で`skin_writer.cc`側にfatal/warningの
  分岐が無い（`symbolskin_writer_t`は`write_obj`をオーバーライドしないため。詳細は
  `src/rules/symbol.rs`冒頭のREJECTEDコメント参照）
- smoke の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。menu/cursor/symbolと全く同じ理由で`skin_writer.cc`側に
  fatal/warningの分岐が無い（`smoke_writer_t`は`write_obj`をオーバーライドしないため。詳細は
  `src/rules/smoke.rs`冒頭のREJECTEDコメント参照）。アニメーション周期・フェード等の
  視覚エフェクトタイミング検証（`smokeuplift`/`smokelifetime`の妥当性等）も、これらが
  `obj=factory`側のフィールドであり`skin_writer.cc`/`skin_writer.h`には一切登場しないため
  対象外（本ツールの対象は`obj=smoke`のスキン画像定義のみ）
- field の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。menu/cursor/symbol/smokeと全く同じ理由で`skin_writer.cc`側に
  fatal/warningの分岐が無い（`field_writer_t`は`write_obj`をオーバーライドしないため。詳細は
  `src/rules/field.rs`冒頭のREJECTEDコメント参照）。生育段階数・生育速度等の妥当性検証
  （画像枚数と`obj=factory`側の`production_per_field`等との整合性）も、これらが`obj=factory`側の
  フィールドであり`skin_writer.cc`/`skin_writer.h`には一切登場しないため対象外
  （本ツールの対象は`obj=field`のスキン画像定義のみ）。参照元の`obj=factory`が実際にこの
  `obj=field`を参照しているかの横断検証は、1ファイル単位の`lint`ではなく`analyze`
  サブコマンドのようなスコープになるため現時点では対象外
- misc の `name`/`copyright` 未指定検証、画像0枚（`image[0]`未指定）警告、および`image[<i>]`の
  途中欠落（"歯抜け"）自体の検出。menu/cursor/symbol/smoke/fieldと全く同じ理由で
  `skin_writer.cc`側にfatal/warningの分岐が無い（`miscimages_writer_t`は`write_obj`を
  オーバーライドしないため。詳細は`src/rules/misc.rs`冒頭のREJECTEDコメント参照）。
  `image[i]`の枚数・インデックスごとの意味（`construction.dat`のコメントが示す
  「0=city building, 1=tourist attraction...」等）の妥当性検証も、これはゲーム実行時ロジック
  （`skin_desc_t`側）の領分であり`skin_writer.cc`/`skin_writer.h`には一切登場しないため対象外
...
- `fmt` の並び替え（デフォルト挙動）でのコメント保持は、直後の`Pair`行に紐づく場合のみ
  対応しています（詳細は上記「並び替えとコメント行の扱い」参照）。紐づけ先が無い孤立コメント
  （不正行の直前、セグメント境界の直前、ファイル/セグメント末尾）は並び替え後の位置が
  一意に決まらないため、引き続き出力から除外されます
...
`tests/real_data_regression.rs` から参照します（`JpClassicTerminal.dat`・`tunnelz.dat`・
`KSN-128op_Rail-yard_0001.dat`・`KSN-128op-OTRP_Rail-yard_0001.dat`。パラメータ展開構文の
実例、`type=station`系の本物のobsolete判定、複数`obj=`連結ファイルの回帰を固定しています）。
**`testdata/real_data/` はサードパーティ製の実アドオンデータのため`.gitignore`対象で、
git管理には含めていません**（ローカル検証専用）。このディレクトリが無い環境
（CI・このリポジトリを新規にクローンした開発者の手元）では、`tests/real_data_regression.rs`
の各テストはフィクスチャ欠如を検知して理由を出力した上で自動的にスキップされ、
...
src/
  ...
  config.rs               lint/fmt/analyze共通設定（TOML）。[rules]診断ルールinclude/exclude
                         ...
                         "fmt-reorder-applied"というcodeで制御する（第11弾で[fmt]
                         セクション自体を廃止）。初回起動時のdat_linter.toml自動生成も
                         ここが担う
  ...
  rules/
    ...
    ground.rs                                obj=ground のRule実装
    menu.rs                                   obj=menu のRule実装
    cursor.rs                                  obj=cursor のRule実装
    symbol.rs                                   obj=symbol のRule実装
    smoke.rs                                     obj=smoke のRule実装
    field.rs                                      obj=field のRule実装
    misc.rs                                        obj=misc のRule実装
