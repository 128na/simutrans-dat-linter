...
citycar/pedestrian/factory/sound/ground の全16 obj種別をカバーしています。building〜factoryの14種は
このプロジェクトが事前に合意した対応範囲で、sound・groundはその計画完了後に追加された対象範囲です
（詳細は下記sound・groundの節を参照）。soundマイルストーンでの`descriptor/writer/`配下の
機械的な棚卸しにより、`ground`（本マイルストーンで対応）に加え`menu`/`cursor`/`symbol`/`smoke`/
`field`/`misc`（`skin_writer.h`定義、いずれも未対応）という計7種の新規obj種別が発見されており、
本マイルストーンはそのうち`ground`のみを対象にした最初の1件です。
...
### `lint` — 静的検証（`obj=building` / `obj=vehicle` / `obj=way` / `obj=good` / `obj=bridge` / `obj=tunnel` / `obj=roadsign` / `obj=crossing` / `obj=way-object` / `obj=ground_obj` / `obj=tree` / `obj=citycar` / `obj=pedestrian` / `obj=factory` / `obj=sound` / `obj=ground`）
...
`crossing`/`way-object`/`ground_obj`/`tree`/`citycar`/`pedestrian`/`factory`/`sound`/`ground` に対応）、未対応の obj 種別では並び替えを行わず元の順序のまま出力します。
...
`src/rules/sound.rs` / `src/rules/ground.rs` 冒頭コメント参照）。
...
ground_obj・tree・citycar・pedestrian・factory・sound・ground のルールは vanilla Simutrans のみで確認済みで、OTRP との個別 diff はまだ行っていません。
...
- building/vehicle/way/good/bridge/tunnel/roadsign/crossing/way-object/ground_obj/tree/citycar/pedestrian/factory/sound/ground
  以外の obj 種別。このプロジェクトが事前に合意した対応obj種別計画（14種）とsound・ground
  マイルストーン（計画完了後の追加、15・16種目）で判明した範囲では、makeobjの
  `descriptor/writer/`配下に`register_writer(true)`でトップレベルobj種別として登録されている
  ものが他に6種存在する: `skin_writer.h`内の`menuskin_writer_t`の`"menu"`・
  ...
  （`ground_writer_t`の`"ground"`は本マイルストーンで対応済み）。
