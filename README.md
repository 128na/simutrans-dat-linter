...
すべてのサブコマンド（`lint` / `fmt` / `analyze`）は明示的に指定する必要があります
...
1ファイルでも error が1件でもあれば、全体の終了コードは失敗（`1`）になります
（単一ファイルを指定した場合は従来通りその1ファイルの結果のみを終了コードに反映し、
集計行も追加しません）。
...
```
src/
  main.rs                clap による CLI 入口（lint/fmt/analyze。lint/fmtは複数ファイル
                         （ディレクトリ再帰・globパターン）・--configに対応。fmtはデフォルトで
                         並び替える（無効化は--no-reorder）。analyzeは--kindで解析種別を選択
                         （現状coupling一種のみ、ワイルドカードarmを持たない網羅match）。
                         --helpの短い説明文はconfigのlanguageに応じて動的に翻訳する）
