...
citycar/pedestrian/factory/sound/ground/menu の全17 obj種別をカバーしています。building〜factoryの
14種はこのプロジェクトが事前に合意した対応範囲で、sound・ground・menuはその計画完了後に追加された
対象範囲です（詳細は下記sound・ground・menuの節を参照）。soundマイルストーンでの
`descriptor/writer/`配下の機械的な棚卸しにより、`ground`・`menu`（いずれも対応済み）に加え
`cursor`/`symbol`/`smoke`/`field`/`misc`（`skin_writer.h`定義、いずれも未対応）という計7種の
新規obj種別が発見されており、本マイルストーンはそのうち`menu`を対象にした2件目です
（`menu`/`cursor`/`symbol`/`smoke`/`field`/`misc`の6種は共通の基底クラス`skin_writer_t`の
サブクラスで、`write_obj`ロジック自体は全種で共有されている。詳細は`src/rules/menu.rs`冒頭コメント
参照）。
...
### `lint` — 静的検証（`obj=building` / `obj=vehicle` / `obj=way` / `obj=good` / `obj=bridge` / `obj=tunnel` / `obj=roadsign` / `obj=crossing` / `obj=way-object` / `obj=ground_obj` / `obj=tree` / `obj=citycar` / `obj=pedestrian` / `obj=factory` / `obj=sound` / `obj=ground` / `obj=menu`）
...
`crossing`/`way-object`/`ground_obj`/`tree`/`citycar`/`pedestrian`/`factory`/`sound`/`ground`/`menu` に対応）、未対応の obj 種別では並び替えを行わず元の順序のまま出力します。
...
`src/rules/sound.rs` / `src/rules/ground.rs` / `src/rules/menu.rs` 冒頭コメント参照）。
...
ground_obj・tree・citycar・pedestrian・factory・sound・ground・menu のルールは vanilla Simutrans のみで確認済みで、OTRP との個別 diff はまだ行っていません。
...
- `lint`: `obj=building`（`type=extension` / `stop` / `depot` 系）、`obj=vehicle`、`obj=way`、`obj=good`、
  ...
  `obj=citycar`、`obj=pedestrian`、`obj=factory`、`obj=sound`、`obj=ground`、`obj=menu`
- `couplings`: `obj=vehicle` の `constraint[prev]` / `constraint[next]`

### 既知の制限（意図的に非対応）

- building/vehicle/way/good/bridge/tunnel/roadsign/crossing/way-object/ground_obj/tree/citycar/pedestrian/factory/sound/ground/menu
  以外の obj 種別。このプロジェクトが事前に合意した対応obj種別計画（14種）とsound・ground・menu
  マイルストーン（計画完了後の追加、15・16・17種目）で判明した範囲では、makeobjの
  `descriptor/writer/`配下に`register_writer(true)`でトップレベルobj種別として登録されている
  ものが他に5種存在する: `skin_writer.h`内の`cursorskin_writer_t`の`"cursor"`・
  `symbolskin_writer_t`の`"symbol"`・`smoke_writer_t`の`"smoke"`・`field_writer_t`の`"field"`・
  `miscimages_writer_t`の`"misc"`
  （`ground_writer_t`の`"ground"`・`menuskin_writer_t`の`"menu"`はいずれも対応済み。
  この6種は共通の基底クラス`skin_writer_t`のサブクラスで`write_obj`ロジック自体は共有されている。
  詳細は`src/rules/menu.rs`冒頭コメント参照）。
