...
設定ファイルの `[fmt]` セクションで、`fmt` が並び替え（慣習的な順序への reorder）を
デフォルトで行うかどうかを切り替えられます。
...
**設定ファイルが無い場合・`reorder` キー未指定の場合のデフォルトは `true`**（並び替える）です。
CLIの `--no-reorder` フラグは、このconfig設定に関わらず**そのプロセスの実行に限り**
強制的に無効化します（優先順位: `--no-reorder`指定 > config設定）。恒久的に並び替えを
無効化したい場合はconfigで `reorder = false` を設定してください。
...
`--no-reorder`（このプロセスの実行に限りconfigの`[fmt] reorder`設定を上書きしてpreserve-order
挙動にする）を指定するか、`dat_linter.toml`の`[fmt] reorder = false`で恒久的に無効化してください
（優先順位: `--no-reorder`指定 > config設定。詳細は下記「出力言語」と同じ設定ファイル節を参照）。
...
src/
  ...
  config.rs               lint/fmt/analyze共通設定（TOML）。[rules]診断ルールinclude/exclude
                         （Diagnostic.code単位）・[general] language・[fmt] reorder
                         （fmtのデフォルト並び替え挙動、未指定時true）の3セクション。
                         初回起動時のdat_linter.toml自動生成もここが担う
