...
- New settings `simutransDatLinter.lint.enable` and `simutransDatLinter.format.enable` (both
  default `true`), letting users who don't have `dat_linter` installed and only want syntax
  highlighting/snippets disable diagnostics and/or Document Formatting independently, instead of
  getting an error popup every time a `.dat` file is opened, saved, or formatted. Turning
  `lint.enable` off clears any diagnostics already shown immediately (not just on the next
  open/save); turning it back on re-lints every currently open `.dat` document without requiring a
  window reload.
