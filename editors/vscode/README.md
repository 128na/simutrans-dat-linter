...
- Key/value completion is powered by `dat_linter keys --format json`, which requires `dat_linter 0.2.0` or later. On an older version, completion is silently disabled (check the "Simutrans dat_linter" output channel for details) while syntax highlighting, formatting, and lint keep working normally.
...
- キー・値の入力補完は `dat_linter keys --format json` を利用しており、`dat_linter 0.2.0` 以降が必要です。それより古いバージョンでは補完機能のみ静かに無効化されます（詳細は "Simutrans dat_linter" 出力チャンネルを参照）。シンタックスハイライト・フォーマット・lint は引き続き通常通り動作します。
