...
- `gh`（GitHub CLI）が使えるとリリース情報の取得が楽になるが、無くても
  `Invoke-RestMethod`でGitHub APIを直接叩けば同じことができる。
...
最新版と比較する場合:
```powershell
gh api repos/128na/simutrans-dat-linter/releases/latest --jq .tag_name
```
...
### 3-1. dat_linter導入
```powershell
$tag = gh api repos/128na/simutrans-dat-linter/releases/latest --jq .tag_name
...
```powershell
$release = gh api repos/simutrans/simutrans/releases --paginate | ConvertFrom-Json |
    Where-Object { $_.tag_name -ne "Nightly" } | Select-Object -First 1
$tag = $release.tag_name
```
（ビルド不要。公式が既にWindows向けビルド済みzipを配布している）
```powershell
$url = "https://github.com/simutrans/simutrans/releases/download/$tag/makeobj-win-60-11.zip"
# ↑ アセット名の"60-11"部分（makeobjのバージョン番号）はリリースごとに変わりうるので、
#   $release.assets から実際のファイル名（"makeobj-win-*.zip"パターン）を拾って使うこと。
#   固定文字列として決め打ちしないこと。
